...
def to_occupancy_map(img: np.ndarray, unknown_val: int = 205) -> np.ndarray:
    """
...
def to_exploration_map(img: np.ndarray, unknown_val: int = 205) -> np.ndarray:
    """
...
def to_target_pose_map(img: np.ndarray, x: int, y: int, dest: str) -> None:
    ...
    cv2.imwrite(dest, target_pose)

def to_ego_pose_map(img: np.ndarray, x: int, y: int, dest: str) -> None:
    """
    Converts a map image to an ego pose map.
    0 for every pixel except the ego's position, which is 255.
    """
    ego_pose = np.zeros_like(img)
    ego_pose[y, x] = 255
    
    cv2.imwrite(dest, ego_pose)

def save_rescuer_pose_map(img: np.ndarray, x: list[int], y: list[int], dest: str) -> None:
    """
...
def to_explorer_pose_map(img: np.ndarray, x: list[int], y: list[int], dest: str) -> None:
    ...
    cv2.imwrite(dest, explorer_pose)
